...
/**
 * Dark chrome: a stadium that stretches toward the thumb, plus a gooey metaball neck while the
 * thumb is still coupled. The thumb circle itself is drawn separately (lighter fill) so the number
 * can ride the finger.
 */
...
/**
 * Classic two-circle metaball (Zhang Xin / Cuberto gooey). Adds a closed bezier "neck" joining [c1]
 * and [c2]. No-ops when the circles are too far apart or one radius is degenerate.
 */
fun Path.addMetaball(
    ...
    val d2 =
        min(v * handleLenRate, (p1 - p3).getDistance() / totalRadius) *
            min(1f, d * 2f / totalRadius)
    val r1d2 = r1 * d2
    val r2d2 = r2 * d2
    val halfPi = pi / 2f
